...
- GitHub's initial `main` qualification run failed before testing because PowerShell 7's inherited `PSModulePath` hid Windows PowerShell's built-in `Get-FileHash`. Commit `908aad9` gives the GitHub job the explicit Windows PowerShell module path without changing local npm scripts. A new branch qualification run is required before treating the GitHub Actions artifact as canonical.
