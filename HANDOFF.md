...
## Customer-skill and operations review remediation - 2026-08-12

- Reviewed implementation commit `0b7e6cc8f497a87449e1392450a9b68a8ec72af1` on branch `codex/resume-infrastructure` closes the verified customer-skill, remote MCP, exact-artifact, and backup-maintenance findings. This handoff entry is the following documentation commit. GitHub remains the source of truth; the conflicted Nextcloud checkout was not modified.
- A fresh agent given only the packaged eight-file skill answered three realistic setup, unsafe-prototype, and custom-header prompts correctly: it refused credentials and database data, preserved normal-search fallback and server-authoritative visibility, classified current/planned/unsupported features truthfully, and did not claim site access or mutation. Deterministic checks are explicitly described as eight route-specific instruction-coverage cases rather than host-output tests.
- The prototype validator now accepts only exact canonical component/source/render tuples and versioned current/planned feature allowlists. It bounds file size, nesting, collections, and errors; opens a regular non-symlink manifest once; reads at most 256 KiB plus one byte from the verified handle; and self-tests unsafe mappings, false capability claims, oversized files, and symlinks.
- Remote MCP hardening now strips expanded credential-like downstream keys, caches unchanged revocation snapshots while immediately observing atomic replacement, bounds tracked clients, body readers, and executions separately, applies explicit HTTP timeouts/header/socket limits, requires both admin Host and direct source address for operational routes, and serializes audit writes with application-enforced 50 MiB rollover and 14 retained files. The remote HTTP service remains undeployed and must not be exposed before its documented external gates pass.
- Root backup scripts now fail closed unless their configuration is a root-owned regular non-symlink with no group/other permissions, then source the already verified file descriptor. A separate weekly maintenance service/timer uses the unrestricted maintenance client and a non-overlapping lock. It must not be installed on `s2` with the append-only daily-writer credential. The existing nightly backup timer remains disabled/inactive pending the previously documented custody and maintenance-client gates.
- Exact-artifact ZIP comparison now rejects wrong roots, traversal/dot/empty segments, backslashes, drive forms, duplicate/case-colliding names, and paths resolving outside the installed root. Standalone packaging and qualification both require all eight customer-skill files and run the semantic customer-skill verifier.
- Verification passed after the final fixes: `pnpm install --frozen-lockfile`; `pnpm test`; two complete `pnpm test:all` runs; `pnpm test:mcp:live`; `pnpm audit --prod --audit-level high`; two successful `pnpm qualify:artifact` runs after correcting a gate-detected PowerShell empty-set annotation; `pnpm verify:customer-skill`; `pnpm verify:infra`; Semgrep `p/security-audit` plus `p/secrets`; Gitleaks on the diff and branch history; and `git diff --check`. MCP is 13/13, PHP is 40/40, and independent final rereview found no remaining actionable remediation issue. Temporary WordPress Application Password inventory is zero.
- Final artifact evidence: 82 files, 458543 bytes, tree `AD58C38A166B054FFDA438DF8F7C37BD789317BFBD695EE019AD314D436B364A`; plugin ZIP `AA261D89874C312F4D5F785F1357678D682BC2B51F8851AB97852B227779B160`; customer skill ZIP `688796524AC75A940A56436D75C5EDCA15501FDF72225804789AD725BB51FA0C`; SBOM `A0A83BCE779E3B28B6FC4F54BFB38466B06FBBB3B410A2AAAD383CFC13BA0334`; manifest `28A013BE569D4B94E35A246E717370D3400E03B255E7BAC1DBE69C31ECF7418E`.
- No VPS, Storage Box, DNS, firewall, proxy, Vault, provider, production WordPress, backup repository, timer, or durable credential state changed during this review. Generated artifacts remain ignored. External qualification blockers and Gate 10 remain open; do not describe the build as released or production-ready.

## Customer-facing AI assistant skill - 2026-08-12

- Implementation commit `bb103c491fd60fd662fb0d34526e305aa347e2b1` on branch `codex/resume-infrastructure` makes `plugin/starfiniti-search/ai/starfiniti-search-assistant/` the canonical, version-matched customer skill. GitHub and the clean clone remain the source of truth; the conflicted Nextcloud checkout was not modified.
- The eight-file guidance-only skill serves store owners and builders across setup, troubleshooting, storefront placement, prototype review, relevance, analytics, and capability questions. It labels material claims `available_now`, `planned`, or `unsupported`; refuses credentials, database dumps, customer/order data, private catalog exports, unredacted logs, raw SQL/provider administration, visibility bypasses, and automatic site mutations; and never claims that it connected to or changed a site.
- Codex and Claude repository entries are thin aliases to the packaged source of truth. The older `starfiniti-prototype-design` name remains a compatibility alias. The WordPress administration page and packaged readme explain Codex installation at `~/.agents/skills/starfiniti-search-assistant`, Claude Code installation at `~/.claude/skills/starfiniti-search-assistant`, and the explicit qualification-build/release boundary.
- Packaging now reproducibly builds both `starfiniti-search.zip` and `starfiniti-search-ai-skill-0.3.0-alpha.1.zip`. The CycloneDX SBOM and release manifest bind both components and both ZIP hashes. Exact-artifact qualification installs the plugin and byte-compares all 82 plugin files plus all eight companion-skill files against the embedded installed copy.
- All five canonical/alias entry points passed the skill-creator structural validator. `verify:customer-skill` passed eight customer scenarios and rejected a deliberately unsafe prototype manifest for five independent violations. `pnpm test` passed 115 requirements/28 specification hashes, architecture/infrastructure, customer-skill checks, PHP 40/40, and MCP 9/9. The full wrapper encountered the already documented local WordPress seed/action-scheduler handoff race; only the exact test processes were stopped, the remaining action was safely drained, and standalone integration, storefront, contracts, lifecycle, both forced-process recoveries, disaster recovery, and release scan all passed.
- `pnpm test:mcp:live` passed discovery, status, configuration, and dry-run planning; independent inventory found zero temporary MCP qualification Application Passwords. `pnpm audit --prod --audit-level high` found no known vulnerabilities. Final `pnpm qualify:artifact` passed in 98 seconds with clean official Plugin Check and all installed contracts/recovery/smoke tests.
- GitHub run `31599138040` passed clean provisioning, deterministic release-evidence generation, the customer-skill gate, PHP 40/40, MCP 9/9, and storefront verification, then exposed a second fresh-database Action Scheduler race: the pending probe observed work, but the group-scoped CLI runner could no longer resolve the group. Commit `326f5accb93a7e4e46301c7e0378be6b1e9e40b4` scopes the test runner to the complete five-hook Starfiniti allowlist instead, preserving isolation without depending on transient group existence. Standalone integration passed in 103.1 seconds; the complete `pnpm test:all` passed in 159.2 seconds; live MCP and the production dependency audit passed; and final exact-artifact qualification passed in 132.6 seconds with unchanged hashes and zero temporary Application Passwords. Fresh GitHub run `31601070970` then passed all stages at branch head `2cd5917edcb01f6e67e82ed25f9b336a07d23e90`, including the repaired qualification suite, exact-artifact installation, and upload; its `starfiniti-search-qualification` bundle is 148841 bytes with digest `sha256:62ddaf0c68753fbea1652c2c6cb3187b6cb71af0a3acf13e25ae2625eeca2af4`.
- Final artifact evidence: 82 files, 452735 bytes, tree `42A09D134886FC5B1EDFE2C069F83FE3BC7CC79F17B7FC7E3C662ACC5F2B457C`; plugin ZIP `7D1C7E1A3DBFBB7B1D3FF65B1BED260CA2465572779F5F303018A7CC8D963642`; customer skill ZIP `74EC9D359666C545F77A3E1E74853B32F552D7EC058A0AD263EC27407EEDED73`; SBOM `9093AF3EA3DD75714F2BFEF58FCA66DD2BAE26CD5DD9EFC67AD061CDDCDD9D6F`; manifest `62D11C8A6F338D2C6526663118BBF9D4AFFD3C6A943D1C8788C2215442F0143D`.
- No VPS, DNS, firewall, proxy, provider, backup, Vault, production WordPress, or durable credential state changed. The generated release artifacts remain ignored. Next safe action: keep the pull request in qualification status and publish the companion skill ZIP only as part of a future qualified release; Gate 10 and other external blockers remain open.

## Cross-agent prototype design skill - 2026-08-12

- The cross-agent prototype skill is committed on branch `codex/resume-infrastructure` at `8b18b18c411d836eab7c91e54ab4726de0eec610`; this handoff update is the following documentation commit. GitHub remains the source of truth and the conflicted Nextcloud checkout was not modified.
- A canonical repository skill now lives at `.agents/skills/starfiniti-prototype-design/`. It makes Codex classify Figma, FigJam, image, HTML, or written UX prototypes as `compatible_now`, `planned`, or `incompatible` against the provider-neutral storefront, canonical request/response contracts, current implementation, accessibility behavior, administrator operation model, and qualification boundary.
- Claude discovers the same contract through `.claude/skills/starfiniti-prototype-design/SKILL.md`, a thin wrapper that points to the canonical `.agents` skill. `CLAUDE.md` delegates repository rules to `AGENTS.md`; no independent Claude copy of the design contract exists.
- The skill includes a compatibility reference, a prototype-manifest template, and a dependency-free validator. The validator requires surface-specific state/interaction coverage, canonical typed bindings, server-authoritative visibility, safe text rendering, desktop/mobile viewports, provider-neutral storefront behavior, current-versus-planned feature separation, and truthful theme-certification claims. Its self-test accepts the compatible example and rejects a deliberately unsafe example for four independent violations.
- `pnpm test` now runs `verify:prototype-skill`, so CI fails if the validator or example contract drifts. Both skill folders passed the skill-creator structural validator. Direct example validation and the self-test passed.
- `pnpm test` passed with real upstream archive/tree verification, 115-requirement and 28-hash verification, architecture/infrastructure checks, prototype-skill self-test, PHP 40/40, and MCP 9/9. `pnpm test:all` passed in 126.3 seconds, including storefront, integration, live schema, lifecycle, both process-kill recoveries, disaster recovery, and release scan. A prior combined wrapper timed out while an orphaned integration process remained; only its exact test process tree was stopped, the isolated integration suite then passed in 65.8 seconds, and the complete standalone rerun passed.
- `pnpm test:mcp:live` passed discovery, status, configuration, and dry-run planning. Independent inventory found zero temporary `Starfiniti MCP qualification` Application Passwords afterward. `pnpm audit --prod --audit-level high` found no known vulnerabilities.
- `pnpm qualify:artifact` passed in 109.5 seconds. The development-only skill did not change the 74-file, 422590-byte plugin tree `2080955E96C7BAE8A42316CB4DE9A823342D5FC4AA86B50EC6748F633FEB80F4`; ZIP `2F2E56DDE074C526BDB9AA672825414D35F5AE749C3950A88CF35B6BD144C8C5`; SBOM `B6E487BB453B672F45830D3A74205DC3B02169677B01B3866047DA903546BD90`; manifest `57B2B9B981D79B1791CF36CB11B3E75E6E0B5FA92A673BE1C095D26B324C827D`.
- GitHub run `31588816172` passed the new prototype-skill self-test, requirements, architecture, infrastructure, PHP, MCP, storefront, and clean runtime/package provisioning, then exposed an Action Scheduler CLI edge after the initial catalog seed had already completed asynchronously: the absent group was reported as an error even though no work remained. Commit `d5e6b36b1697214e89907fdf59d73cd8ef576217` adds a read-only pending-action probe, skips the group runner only when pending count is zero, and still fails when execution leaves work. Both a real pending group and a deliberately absent group passed locally. The final `pnpm test:all` passed in 151.2 seconds, live MCP and dependency audit passed with zero temporary credentials, and `pnpm qualify:artifact` passed in 141.3 seconds with unchanged hashes. GitHub must rerun the fix before the branch is green.
- No VPS, DNS, firewall, proxy, provider, backup, Vault, credential, or production WordPress state was changed. Next safe action: require GitHub qualification on the pushed scheduler-fix commit, then invoke `$starfiniti-prototype-design` from Codex or the same named Claude skill for the first real prototype.
...
- GitHub run `31518899927` passed clean provisioning and release-evidence generation, then showed that a brand-new database had no Starfiniti Action Scheduler group when the integration harness first attempted to drain it. Commit `f66ad9f` explicitly invokes the plugin's bounded initial catalog seeder only while the seed-complete option is absent, then retains the group-scoped queue drain. The forced fresh-seed integration run and the complete recorded-evidence CI-mode `pnpm test:all` passed locally afterward. GitHub must qualify this commit before its Actions artifact is canonical.
