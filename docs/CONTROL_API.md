...
`apps/search-ops-mcp` is the official v2 SDK TypeScript adapter over this boundary. Its local stdio mode is explicitly process-trusted, and its modern-only Streamable HTTP mode creates a fresh principal-bound server per request. Both expose the same five fixed tools plus tenant-scoped status resources; neither has a generic URL, SQL, shell, raw provider, or execution primitive. Registered destinations are exact versioned Starfiniti endpoints, credentials are server-side environment references, caller tokens are never forwarded, downstream text is marked and bounded as untrusted data, and every call produces a secret-free MCP audit record. The real-localhost gate proves authenticated status, configuration, and dry-run planning before revoking its temporary Application Password.

The remote implementation publishes RFC 9728 metadata, validates Auth0 discovery and PKCE S256/DCR support, verifies RFC 9068 RS256 access tokens through JWKS, enforces exact issuer/audience and a 15-minute maximum lifetime, checks a namespaced tenant claim and local `jti` revocation, bounds request rate/body/concurrency, and provides private health/readiness/metrics routes. Public deployment remains disabled until Auth0, TLS, firewall, proxy routing, monitoring, and Codex/Inspector/official-client interoperability pass on the certification environment. See `apps/search-ops-mcp/README.md`.
