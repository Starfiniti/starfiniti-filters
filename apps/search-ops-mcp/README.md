...
Start remote mode with `pnpm --filter @starfiniti/search-ops-mcp build` followed by `pnpm --filter @starfiniti/search-ops-mcp start:http`. `http.example.env` documents every required variable. The revocation file is a JSON object shaped like `revocations.example.json`; update it by atomic replacement. Before every token decision the service verifies the open file's identity and metadata, reparses only a changed snapshot, and fails closed if the file is unavailable or invalid.
...
The included deployment examples bind the application to the private network, publish only `/mcp` and the OAuth well-known routes through Caddy, and require both an admin hostname and an exact direct monitoring source address for `/livez`, `/readyz`, and `/metrics`. Do not proxy the admin routes publicly. Configure `STARFINITI_MCP_TRUSTED_PROXY_ADDRESSES` only with the exact Caddy source address; otherwise forwarded client addresses are ignored.

The service sets explicit header, total-request, body-read, keep-alive, header-count, per-socket request, body-admission, execution-concurrency, per-client rate, and tracked-client limits. The append-only audit writer serializes writes, rolls before exceeding 50 MiB, retains at most 14 prior files, and never makes audit failure optional. Alert on service-volume usage; the default retained audit bound is approximately 750 MiB.
